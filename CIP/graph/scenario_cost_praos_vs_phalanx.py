...
# Add annotation for the delta at rho = 50 (using Owl Survey Phi_max and Owl Survey Phi_min)
rho_idx_50 = np.argmin(np.abs(rho - 50))
delta_log_cost_50 = log_cost_owl_survey_phi_max[rho_idx_50] - log_cost_owl_survey_phi_min[rho_idx_50]
mid_y_50 = log_cost_owl_survey_phi_max[rho_idx_50] - (delta_log_cost_50 / 2)

plt.annotate('', xy=(50, log_cost_owl_survey_phi_max[rho_idx_50]), xytext=(50, log_cost_owl_survey_phi_min[rho_idx_50]),
             arrowprops=dict(arrowstyle='<->', color='purple', lw=1, shrinkA=0, shrinkB=0))
plt.text(53, mid_y_50, f'$\\Delta \\log_{{10}}(\\text{{Cost (USD)}}) \\approx {delta_log_cost_50:.1f}$',
         fontsize=12, color='purple', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
         verticalalignment='center')

# Add annotation for the delta at rho = 100 (using Owl Survey Phi_min and Owl Survey Praos)
rho_idx_100 = np.argmin(np.abs(rho - 100))
delta_log_cost_100 = log_cost_owl_survey_phi_min[rho_idx_100] - log_cost_owl_survey_praos[rho_idx_100]
mid_y_100 = log_cost_owl_survey_phi_min[rho_idx_100] - (delta_log_cost_100 / 2)

plt.annotate('', xy=(100, log_cost_owl_survey_phi_min[rho_idx_100]), xytext=(100, log_cost_owl_survey_praos[rho_idx_100]),
             arrowprops=dict(arrowstyle='<->', color='purple', lw=1, shrinkA=0, shrinkB=0))
plt.text(103, mid_y_100, f'$\\Delta \\log_{{10}}(\\text{{Cost (USD)}}) \\approx {delta_log_cost_100:.1f}$',
         fontsize=12, color='purple', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
         verticalalignment='center')

# Add annotation for the delta at rho = 150 (using Owl Survey Phi_min and Ant Glance Praos)
rho_idx_150 = np.argmin(np.abs(rho - 150))
delta_log_cost_150 = log_cost_owl_survey_phi_min[rho_idx_150] - log_cost_ant_glance_praos[rho_idx_150]
mid_y_150 = log_cost_owl_survey_phi_min[rho_idx_150] - (delta_log_cost_150 / 2)

plt.annotate('', xy=(150, log_cost_owl_survey_phi_min[rho_idx_150]), xytext=(150, log_cost_ant_glance_praos[rho_idx_150]),
             arrowprops=dict(arrowstyle='<->', color='purple', lw=1, shrinkA=0, shrinkB=0))
plt.text(153, mid_y_150, f'$\\Delta \\log_{{10}}(\\text{{Cost (USD)}}) \\approx {delta_log_cost_150:.1f}$',
         fontsize=12, color='purple', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
         verticalalignment='center')
